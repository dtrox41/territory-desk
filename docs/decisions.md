...
## D-033 — Save one revalidated handoff before attempting alerts

- Date: 2026-08-24
- Decision: Implement fictional Lead Creation as a four-step Route, Customer, Opportunity, and Review & Send workflow. Require exact service and five-digit ZIP routing, one current eligible recipient, explicit directory-mismatch resolution, server-shaped revalidation at send time, an active-session idempotency key, duplicate warning with reasoned override, and handoff persistence before in-app and simulated-SMS attempts. Transfer territory, directory, and create-another context through protected navigation state rather than customer or recipient URL parameters.
- Reason: A fast form is valuable only when it prevents wrong-recipient handoffs, fabricated contact details, accidental duplicate submissions, silent routing changes, and false success when a notification channel fails. Field representatives also need a short smartphone workflow that retains input through validation and clearly separates customer timing from the recipient response target.
- Alternatives considered: One long form; send directly from Territory or Directory; accept a representative name without ZIP validation; put recipient or customer context in the URL; persist drafts in browser storage; treat SMS success as the handoff save; retry by creating a new record; rank ASAP leads ahead of approved required actions; add attachments, revenue priority, or AI-generated notes.
- Consequences: Global entry begins blank; Territory entry carries a versioned routing snapshot but still rechecks it; Directory entry carries only a stable representative ID and displays any mismatch before the current assignment may be accepted; open, ambiguous, stale, missing, self, and changed routes fail closed; the customer may explicitly state that contact information is not yet available; Review & Send cannot be skipped; repeated submission with one key returns one fictional handoff; SMS failure preserves the saved record and instructs the sender not to resubmit. Production still requires authenticated server validation, protected persistence, company retention policy, and a funded approved SMS provider.
- Reversible: Partially. Copy, fixture scenarios, field grouping, and responsive layout can change through field testing, but one-recipient/one-department creation, exact routing revalidation, URL privacy, review confirmation, idempotency, save-before-notification ordering, and truthful partial-failure reporting remain mandatory.

## D-034 — Make Leads a personal, ranked work queue

- Date: 2026-08-24
- Decision: Implement `/leads` as the current representative's personal My Work workspace with six explicit views: Action Required, Waiting on Others, Received, Sent, In Progress, and Completed. Use the approved eight-level required-action hierarchy, one visible action per handoff, safe identifying-field search that remains out of the URL, allowlisted non-sensitive filters, bounded 20-record pagination, independent count failure, stale/offline visibility, and detail-first actions. Keep team-wide manager oversight in `/insights` and do not broaden Leads records based on a manager visual state.
- Reason: Representatives need one reliable place to find the peer handoffs they owe, are waiting on, own, or completed. Combining personal execution with team-wide surveillance, sorting by value or rep performance, or placing status-changing commands on dense cards would increase missed work, accidental actions, privacy exposure, and role confusion.
- Alternatives considered: One undifferentiated lead table; a manager-wide list mode; value, urgency, or performance scoring; raw notification ordering; one-tap Accept, Decline, Complete, or Reassign from cards; unbounded infinite scroll; customer/contact/note search in the URL; duplicate action rows for one handoff.
- Consequences: Home and Leads share a five-item Action Required truth and a two-item Waiting truth in the fictional adapter; managers use the same personal Leads semantics and a separate authorized Insights surface for team exceptions; search is active-session-only; customer contacts, addresses, full notes, and raw provider errors remain off cards; Sent loads 20 records first and expands only on request; disconnected users may read already loaded authorized cards but cannot perform a mutation from this list. Lead Detail remains the next workflow checkpoint for consequential status and ownership commands.
- Reversible: Partially. Fixture volume, card density, labels, filter choices, and pagination size can change after measured field testing, but personal scope, deterministic action ranking, detail-first mutations, privacy-safe URL/search behavior, bounded loading, truthful failure states, and separate manager oversight remain mandatory.

## D-035 — Make Lead Detail the explicit collaboration command boundary

- Date: 2026-08-24
- Decision: Implement `/leads/:leadId` as the authorized collaboration workspace with exactly one current primary action, separate status/current-owner/requested-recipient/action-owner dimensions, idempotent first-view tracking, reviewed Accept/Need Information/Decline commands, version and idempotency protection, structured follow-up continuity, append-only activity, independent block failures, and generic non-disclosing unavailable behavior. Keep offline, stale, mismatched, or conflicting records readable but non-writable.
- Reason: The product cannot repair cross-department communication if opening a record is mistaken for responding, ownership is inferred from status, a completed task has no next commitment, or notification delivery is treated as the business record. Consequential actions need context, explicit review, concurrency protection, and a durable shared history.
- Alternatives considered: One-tap response from list cards; treat view as acknowledgment; combine status and owner; mark a lead up to date after any activity; allow overdue follow-up completion without a next-action requirement; overwrite activity; hide the whole lead when a secondary block fails; disclose whether an unauthorized identifier exists; allow stale or offline writes; connect real integrations during the public fictional prototype.
- Consequences: Representatives can see who owes what and why, send an explicit response, maintain one structured next action, and share progress without silent lifecycle changes. The typed service boundary can later move commands into authenticated transactions. Sender information/correction, manager reassignment, outcome commands, and production integrations remain separate checkpoints rather than partially implemented controls.
- Reversible: Partially. Fixture text, card density, filters, activity page size, and dialog copy can change through field testing, but explicit action ownership, separate lifecycle dimensions, view-versus-response separation, reviewed commands, concurrency/idempotency, append-only history, fail-closed access, and read-only stale/offline behavior remain mandatory.

## D-036 — Keep notification read state separate from sales-work state

- Date: 2026-08-24
- Decision: Implement `/notifications` as the current user's newest-first event history with four categories, separate Unread Only filtering, permission-minimized cards, bounded pagination, idempotent Mark Read, confirmed Mark All Read, safe new-arrival refresh, independent count/list failures, and offline/stale write blocking. Mark a notification read after a linked Lead Detail open only when authorized core loading succeeds. Keep bell count, Leads action count, lead view, response, status/ownership, and SMS/email delivery as separate state dimensions.
- Reason: The app will recreate the existing communication problem if representatives cannot tell the difference between seeing an alert and completing the sales obligation behind it. A reliable center must surface what changed without becoming another ranked work list, creating self-notification noise, exposing provider details, or falsely clearing business work when unread indicators change.
- Alternatives considered: Derive the bell from Action Required; mark all notifications read when the screen opens; mark read on scroll or link preview; execute Accept/Decline/Complete from cards; place free-text notification search in the first release; rank cards by action urgency; expose raw SMS delivery attempts; use infinite scroll; retain a stale count offline; disclose inaccessible destination details.
- Consequences: The shell can truthfully show exact unread meaning, zero, 99+, or unavailable without changing the Leads badge. Users can review event history, clear personal indicators explicitly, and open the current authorized lead state. A full reload may reset fictional in-memory read state; production requires protected persistence, recipient reauthorization, retention/deletion policy, command correlation, and real delivery integrations before the same behavior is trusted with real data.
- Reversible: Partially. Fictional messages, category density, card composition, pagination size, and responsive grid can change after field testing, but event-versus-work separation, read-versus-view/response separation, confirmed bulk read, permission filtering, authorized-open gating, count independence, URL privacy, and offline/stale safeguards remain mandatory.

## D-037 — Make Manager Insights a scoped exception workspace, not a leaderboard

- Date: 2026-08-24
- Decision: Implement `/insights` as a separately authorized fictional Team Insights workspace with one versioned metric result, 7/30/90-day allowlisted filters, deduplicated Needs Attention counts, the two approved workflow KPIs, visible denominators and exclusions, diagnostic guardrails, department-pair small-sample suppression, bounded supporting records, and Lead Detail drill-downs that restore the manager's prior context. Activate the manager shell variation only for the current local fictional preview while keeping representative denial and manager scope states independently tested.
- Reason: Managers need to remove cross-department workflow bottlenecks without turning lead collaboration into employee surveillance. A trustworthy dashboard must reconcile each count to authorized records, distinguish incomplete or low-volume evidence, preserve personal My Work, and keep the one-business-day response rule as a workflow deadline rather than a performance grade.
- Alternatives considered: Add team records to personal Leads; rank representatives or departments; make raw lead volume the hero metric; display fixed fictional percentages; show `0%` for no eligible data; compare every small group; infer qualified progression or revenue without Dynamics; allow bulk lead mutations from the dashboard; accept manager scope from URL parameters; keep prior results after permission removal; compress a wide table into the phone layout.
- Consequences: The fictional manager preview shows valid, insufficient-volume, and quality-blocked comparisons; every KPI exposes its numerator, denominator, exclusions, period, definition, and supporting records; exception lists perform navigation only; authorization loss clears prior results; phone and laptop use the same information model. Production still requires protected authentication, server-enforced scope, business-calendar validation, protected persistence, company cache policy, audited manager commands, and Dynamics reconciliation.
- Reversible: Partially. Fictional fixture mix, card density, wording, responsive breakpoints, and safe filter options may change after field testing, but separate personal/team scope, source-backed calculations, unique attention counts, visible denominators, no ranking, small-sample and data-quality suppression, drill-down reconciliation, fail-closed access, URL privacy, and Dynamics gating remain mandatory.

## D-038 — Enforce one ephemeral fictional session across the integrated Preview

- Date: 2026-08-24
- Decision: Connect the credential-free demo personas to one application-wide in-memory session boundary. Resolve that boundary before rendering protected routes; return signed-out direct links through an allowlisted path-only `returnTo`; derive manager navigation and Profile roles from the selected persona; deny `/insights` to non-managers; and clear the session before protected content can be reopened after sign-out. Do not persist the demo session in cookies, local storage, session storage, a URL, or the service worker.
- Reason: The completed screens were individually safe, but the shell still hard-coded manager presentation and protected routes opened without the fictional sign-in flow. Leaving that gap would make the prototype misleading and would invalidate manager-access, direct-link, refresh, and sign-out testing.
- Alternatives considered: Keep the manager flag hard-coded; expose every route in the public prototype without sign-in; store the selected persona in local or session storage; put role or return context in unrestricted URL parameters; create a fake password or token; delay integration until GitHub publication.
- Consequences: Representative and manager previews now differ consistently; direct protected paths preserve only an approved destination pathname; query strings and fragments are intentionally dropped at the sign-in boundary; a full refresh requires choosing a demo persona again; Profile cannot claim manager access for a representative; and browser Back after sign-out cannot restore the protected workspace. This is still presentation-level prototype enforcement—Production remains blocked on company-approved server authentication, authorization, and revocation.
- Reversible: Partially. A future protected server session will replace the in-memory adapter, but pre-render protection, path allowlisting, least-privilege role presentation, manager denial, non-persistent public demo state, and sign-out clearing remain mandatory.
